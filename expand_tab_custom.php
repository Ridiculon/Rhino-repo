<?php
	...
	for($j = 0; $j < $n; $j++){
			if($item-1 < $j)
				echo '<div class="expandable-tab" id="'.($j+1).'" style="top: '.($j*160 + 230).'px">';
			else if($item-1 == $j)
			...
			if($item-1 == $j){
				echo '<div class="expandable-tab-header-col" id="header'.($j+1).'" onclick="loadurl(\'collapse_tab_custom.php\', 0, '.($j+1).', '.$n.',\''.$user.'\', \''.$q.'\' )">';
				...
				@ $db = mysql_pconnect('localhost', 'userbasic', 'user8asic');
					...
					else
					{
						...
						if($num_results > 0)
						{
							...
							echo '<div style="position: absolute; right:30px"><a href="./list.php?query='.$queries[$j].'">See all</a></div>';
							echo '</div>';
